...
DEFAULT_DB_URL = "postgresql://postgres.cyospfvoadrnfrkbqaeb:wfZ&&A6YgMuBW9E@aws-0-eu-central-1.pooler.supabase.com:5432/postgres"
...
def cargar_vehiculos():
    ...
    try:
        conn = get_connection()
        c = conn.cursor()
        c.execute("SELECT matricula, tipo, conductor, km, itv, seguro, aseguradora, prox_mant_km, prox_mant_fecha, intervalo_km, estado FROM vehiculos ORDER BY matricula ASC;")
        rows = c.fetchall()
        vehiculos = []
        for r in rows:
            vehiculos.append({
                ...
                "prox_mant_fecha": r[8], "intervalo_km": r[9], "estado": r[10]
            })
        return vehiculos
...
def cargar_historial_taller_vehiculo(matricula):
    ...
    try:
        conn = get_connection()
        c = conn.cursor()
        c.execute("SELECT orden, fecha_in, fecha_out, taller, trabajos, km, coste, factura FROM taller WHERE matricula = %s ORDER BY orden DESC;", (matricula,))
        rows = c.fetchall()
        registros = []
        for r in rows:
            registros.append({
...
def registrar_taller_movil(orden, fecha_in, fecha_out, matricula, taller, trabajos, km, coste, factura):
    conn = None
    try:
        conn = get_connection()
        c = conn.cursor()
        c.execute("""
            INSERT INTO taller (orden, fecha_in, fecha_out, matricula, taller, trabajos, km, coste, factura)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);
        """, (orden, fecha_in, fecha_out, matricula, taller, trabajos, km, coste, factura))
...
if veh_actual:
    ...
    with tab2:
        ...
        with st.form("form_taller_movil"):
            ...
            t_trabajos = st.text_area("Trabajos Realizados (neumáticos, pastillas de freno, aceite...):")
            ...
            if btn_guardar_taller:
                if not t_trabajos.strip() or not t_taller.strip():
                    ...
                    orden_id = f"TAL-MOV-{datetime.datetime.now().strftime('%m%d%H%M')}"
                    if registrar_taller_movil(orden_id, t_fech_in.strftime("%Y-%m-%d"), t_fech_out, matricula_seleccionada, t_taller, t_trabajos, t_km, t_coste, t_factura):
        ...
        if not historial:
            ...
            for h in historial:
                with st.expander(f"Orden: {h['orden']} ({iso_a_ddmmyyyy(h['fecha_in'])}) - {h['taller']}"):
                    st.markdown(f"**Trabajos:**\n{h['trabajos']}")
                    st.text(f"Km: {h['km']:,} | Coste: {h['coste']:.2f} € | Factura: {h['factura']}")
